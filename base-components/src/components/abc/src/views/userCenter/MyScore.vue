      ...
      <template v-for="(monthItem,index) in monthList">
        ...
        >
          <div class="detail-head">
            ...
            <div
              @click="showDetail(monthItem)"
              :class="monthItem.isHidDetail?'':'icon_show'"
              class="iconfont flex_icon"
            >&#xe606;</div>
          </div>

          <template v-if="monthItem.isGetDetail">
            <div
              v-show="!monthItem.isHidDetail"
              v-for="(detailItem,index) in monthItem.detailList"
              :key="index"
              class="detail-item"
            >
              <div class="item-userinfo_state">
                <div class="item-userinfo">
                  ...
                  <div class="item-user_state">
                    <img
                      class="img-user_state"
                      :src="detailItem.certification_status == 1 ? isAutoImg : noAutoImg "
                      alt=""
                    >
                  </div>
                </div>
                ...
                <div
                  v-if="detailItem.refund_status==1"
                  class="item-count"
                >
                  +100元 <span class="count_text">已结算</span>
                </div>
                <div
                  v-if="detailItem.refund_status==0 && detailItem.certification_status == 1 "
                  class="item-count"
                >
...
export default {
  ...
  methods: {
    async showDetail(monthItem) {
      monthItem.detailList.length !== 0
        ? (monthItem.isGetDetail = true)
        : (monthItem.isGetDetail = false);
    ...
    async getInvitationMonthInfo(monthItem) {
      try {
        let monthDetail = await this.$axios
          .get(this.$api.invitationMonthInfo, {
            params: {
        ...
        monthItem.detailList.forEach(ele => {
          ele.invitation_time !== undefined
            ? (ele.invitation_time = this.$commonFn.StrDateFormat(
                ele.invitation_time,
                1
              ))
            : null;
          ele.certification_time !== undefined
            ? (ele.certification_time = this.$commonFn.StrDateFormat(
                ele.certification_time,
                1
              ))
            : null;
        });
    ...
    async getInvitationInfo() {
      let invitationInfo = this.$axios
        .get(this.$api.invitationInfo, {
          params: {
            token: this.userInfo.token
          }
        })
