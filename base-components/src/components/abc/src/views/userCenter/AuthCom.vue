<template>
  <div id="qiyerenzheng" ref="wrapper">
    <div class="certified">
      <h3 class="title">上下链企业认证</h3>
      <h4 class="com-name">{{comName}}</h4>
      <div
        class="expiryDate"
      >年审时间：上下链企业认证有效期为一年，当前企业账号的企业认证到期时间是{{expires.year }}年{{expires.month }}月{{expires.day }}日，请在{{a }}年{{b }}月{{c }}日至{{expires.year }}年{{expires.month }}月{{expires.day }}日完成年审。</div>
      <span
        class="btn review"
        ref="btnRview"
      ...
      <ul class="list">
        ...
        >
          ...
          <div class="item-part right">
            <span class="title-r" ref="stateSpan">{{item.status}}</span>
  ...
  methods: {
    async initData() {
     ...
     console.log(`=======`,this.$store.state.company);
      let company_id = JSON.parse(localStorage.getItem("authComInfo"))
        .enterpriseid;
      let pData = {
        token: this.token,
        company_id
