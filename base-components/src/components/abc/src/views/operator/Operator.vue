    ...
    <div class="pushTemplateList">
      ...
      <ul class="list">
        ...
        <li
          ...
          :key="index"
          v-show="item['审核意见'].split('--')[0]=== '审核通过' || item['审核意见'].split('--')[0]=== '审核不通过'"
        >
          ...
          <p class="i-add-ok">
            ...
            <textarea
              ...
              cols="30%"
              type="text"
              :value='item.itemData.sms_audit_content'
            />
            </p>
...
export default {
  ...
  methods: {
    ...
    async freshInfo() {
      // 获取审核列表数据
      let { data: auditList } = await this.$axios.get(
        this.$api.getAuditTemList
      );
    ...
    async isAgreeCommonFun(item, aduitState, str) {
      ...
      let { data: auditList } = await this.$axios
        .post(this.$api.setauditTemState, data)
        .catch(rej => {
