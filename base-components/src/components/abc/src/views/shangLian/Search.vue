<template>
  ...
  <div id="search_page">
    <div class="m-search_page-beside_btn_g">
      ...
      <div
        @click="cutState(0)"
        :class="$route.name!=='searchCompany'?'btn_unactive':''"
        class="u-btn_search_company"
      >
      ...
      <div
        @click="cutState(1)"
        :class="$route.name!=='searchGlobal'?'btn_unactive':''"
        class="u-btn_global_search"
      >
        <div class="btn_content">
          <i class="iconfont btnicon">&#xe608;</i>
          全网搜索
...
export default {
  ...
  watch: {
    $route(to, from) {
      ...
      this.transitionName =
        to.path == this.myPath ? "slide-right" : "slide-left";
    }
  }
};
</script>
