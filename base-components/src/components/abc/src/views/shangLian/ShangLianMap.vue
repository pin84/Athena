...
import NewChart from "@/components/shangLian/mapTest";
...
export default {
  ...
  async beforeCreate() {
    //第一次到访提示 s
    let flag = await Utils.isFirstVisit(this.$api.isShowTips, "企业客库");
  ...
  methods: {
    async showTips() {
      ...
      const user_id_i = queryObj.user_id_i; // 邀请人的id
      const user_id = this.$store.state.loginInfo.userInfo.user_id; //被邀请人的id
      const token = queryObj.token; // 邀请人的token
      const company_id = queryObj.comID; // 邀请人的公司id

      // if (Number(user_id) == Number(user_id_i)) {
      //   console.log(`=====id 相同==`);
      //   this.$router.push("./cooperation");
      //   return;
      // }

      let res = await this.addmember(user_id_i, token, user_id, company_id);
      ...
      const str_1 = `${queryObj.comname}   "${queryObj.username}"   邀请你加入上下链，共享企业资源。`;
      const str_2 = `"${queryObj.comname}"  团队协同人员已满，请联系管理员或认证新的企业。`;
      const str_3 = `您已加入  "${companyname}"  团队协作。如需加入其他认证企业，请先解绑当前认证企业后再加入。`;
      ...
      if (queryObj.behavior === "invitation") {
        this.$store.commit("showPop", {
          ...
          config: {
            ...
            confirm: () => {
              //添加到当前企业
              if (str === str_1) {
                this.addmember(user_id_i, token, user_id, company_id, true);
                setTimeout(() => {
                  this.$router.push("./cooperation");
                }, 1000);
              }
    ...
    //flag ：Boolear  为true时后台把用户加到数据里，false时，只是返回状态
    async addmember(user_id_i, token, user_id, company_id, flag = false) {
      let member = {
        user_id_i,
        token,
        user_id,
      ...
      let { data } = await this.$axios.post(this.$api.addMember, member);
      return data;
    },
