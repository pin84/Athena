<template>
  <div id="msg-search" my-index="precisionSearch">
    <div class="mask" v-if="isShowMask"></div>

    <div id="main-body">
      <ul class="select-list">
        <li class="item" @click="showIndustryList">
          <span class="text">行业</span>
          <span class="ri">{{selsectIndustry || '全行业'}}</span>
        </li>
        <AreaSelect
          @selectArea="selectIndustry"
          :dataList="industryList"
          :isShow="isShowIndustryList"
        />

        <li class="item" @click="showSelectList">
          <span class="text">省份</span>
          <span class="ri">{{selectedProvince || '全国'}}</span>
        </li>
        <AreaSelect @selectArea="selectProvince" :dataList="province" :isShow="isShowProvinceList" />
        <!-- <span class="area-box" ref="areaBox">
        </span>-->
        <li class="item" @click="showSlectCity">
          <span class="text">城市</span>
          <span class="ri">{{selectedCity || '全选'}}</span>
        </li>
        <AreaSelect
          @selectArea="selectCity"
          :dataList="cities"
          :isShow="isShowSelectCity && cities.length !== 0 && selectedProvince!=='全国'"
        />
        <!-- <span class="area-box" ref="areaBox">
        </span>-->
        <li class="item">
          <span class="text">主营产品/服务</span>
          <input
            class="item-input"
            placeholder="请输入主营产品/业务"
            v-model="business"
            @focus="onFocus"
            @blur="onBlur"
          />
        </li>
        <li class="item">
          <span class="text">企业名称关键词</span>
          <input
            class="item-input"
            placeholder="请输入企业名称包含的关键词"
            @focus="onFocus"
            @blur="onBlur"
            v-model="inputCompanyName"
          />
        </li>
      </ul>

      <button class="searchBtn" @click="search">全网搜索</button>
    </div>
    <template v-if="resultList.length!==0">
      <section class="result-list">
        <p class="total">
          <span class="num">共{{comNum}}家企业</span>
          <span class="next-page" @click="nextPage">下一页</span>
        </p>

        <ul class="list sp" ref="hiddenList" v-if="resultList.length">
          <li v-for="(item,index) in resultList" :key="index" class="item sp-i">
            <span>{{index+1}}、</span>
            <div class="text">
              <span class="item-name">{{item.enterprises}}</span>
              <span class="item-phone">[{{item.mobile}}]</span>
            </div>
          </li>
          <!-- <span v-if="resultList.length > 9">......</span> -->
        </ul>
        <ul v-show="isShowNoResultTips" style="margin-top:0.3rem;">暂未搜索到匹配的企业，请重新检索</ul>
      </section>
      <section class="btn" :style="botBtnStyle">
        <span class="btn-text" @click="sendMsg">发送短信触客</span>
        <!-- <span
          class="btn-video"
          @click="sendVideo"
        >发送视频短信</span>-->
      </section>
    </template>

    <PushMsg />
    <SelectTem />
    <MsgTemplate />
  </div>
</template>



<script>
import { commonFnMixin } from "@/assets/js/mixins.js";
import AreaSelect from ".././../components/common/AreaSelect";
import SelectTem from "@/components/pop/pushTemplate/SelectTem";
import MsgTemplate from "@/components/pop/pushTemplate/MsgTemplate";
const PushMsg = resolve => {
  import("@/components/pop/pushMsgVideo/PushMsg").then(module => {
    resolve(module);
  });
};
import Utils from "../../utils";
export default {
  mixins: [commonFnMixin],

  data() {
    return {
      selsectIndustryID: "", //保存当前选择的行业id
      selsectIndustry: "", //保存当前选择的行业
      cityList: [], //页面面初始化时拿回来的城市列表
      inputCompanyName: "",
      isShowIndustryList: false,
      isShowSelectCity: false,
      isShowProvinceList: false, //显示省份列表
      isShowNoResultTips: false,
      selectedProvince: "", //当前选择的省份
      selectedCity: "", //当前选择的城市
      isShowMask: false, //
      comNum: "" || 0, //搜索到的企业数
      comPhoneNum: "" || 0, //搜索到的手机数
      // inputPlaceholder: "请输入主营产品/业务",
      business: "", //主营

      industry: {
        //当前选择的行业
        id: "",
        name: ""
      },
      industryList: [], //保存后台取回来的行业列表
      resultList: [],
      curPageNum: 1, //当前页面
      //当选择全国时。把这组数据传给后台
      // allArea:
      //   "北京市,天津市,上海市,重庆市,河北省,山西省,辽宁省,吉林省,黑龙江省,江苏省,浙江省,安徽省,福建省,江西省,山东省,河南省,湖北省,湖南省,广东省,海南省,四川省,贵州省,云南省,陕西省,甘肃省,青海省,台湾省,内蒙古自治区,广西壮族自治区,西藏自治区,宁夏回族自治区,新疆维吾尔自治区",
      topNum: 0,
      botNum: 0,
      province: [], //全存全国的省份
      cities: [
        //选择省份后存该省份下的所有城市
        // {
        //   id: 1,
        //   label: "广州"
        // }
      ]
    };
  },
  components: {
    AreaSelect,
    PushMsg,
    SelectTem,
    MsgTemplate
  },

  async created() {
    this.getIndustry();
    this.getAllCity();

    // let flag = await commonFn.isFirstVisit(
    //   this.$api.isShowTips,
    //   this.$route.meta.remark
    // );
    // if (flag) {
    //   this.$router.push({
    //     path: "../tipmain",
    //     query: { num: [4], originUrl: this.$route.fullPath } //4是第四张图片
    //   });
    // }
  },

  mounted() {
    window.addEventListener("scroll", this.handleScroll, true);
  },
  computed: {
    botBtnStyle() {
      let bottom = this.botNum;

      return {
        bottom
      };
    }
  },
  methods: {
    selectIndustry(industry) {
      this.selsectIndustry = industry.join();
      this.isShowIndustryList = false;

      let selectIndustry = this.industryList.filter(item =>
        industry.includes(item.label)
      );
      this.selsectIndustryID = selectIndustry.map(item => item.id).join();
    },
    selectCity(city) {
      this.selectedCity = city.join();
      this.isShowSelectCity = false;
    },
    handleScroll: function() {
      let clientHeight =
        document.documentElement.clientHeight || document.body.clientHeight;

      let scrollTop =
        document.documentElement.scrollTop ||
        window.pageYOffset ||
        document.body.scrollTop; // div 到头部的距离

      // this.topNum = scrollTop+'px';
      this.botNum = -scrollTop + "px";
    },

    sendVideo() {
      this.$toast("此功能暂未开放，敬请期待");
    },
    sendMsg() {
      if (this.comNum === 0) {
        return this.$toast("请先搜索相对应的企业");
      }
      // 推送短信
      let showPushMsg = () => {
        this.$store.commit("showPop", {
          popName: "pushMsg",
          params: {
            saveParams: true,
            freshOtherFunc: function() {},

            companyCount: this.comNum,
            hasPhoneCount: this.hasPhoneCount,
            companyList: this.resultList,
            hasPhoneCompanyList: this.resultList,
            bussiness: this.business,

            // 此部分参数仅为了发送短信时的接口所用到的数据 s
            industryid: this.selsectIndustryID,
            provinces:
              this.selectedProvince == "全国" ? "" : this.selectedProvince,
            logo: "down",
            up_industry: "",
            sendType: "chukeMsg"
            // 此部分参数仅为了发送短信时的接口所用到的数据 e
          }
        });
      };

      this.identityAuth(() => {
        showPushMsg();
        this.isFirstVisit();
      });
    },

    //第一次到访提示 s
    async isFirstVisit() {
      let flag = await Utils.isFirstVisit(
        this.$api.isShowTips,
        // this.$route.meta.remark
        "全网搜索推送短信"
      );
      if (flag) {
        this.$router.push({
          path: "../tipmain",
          query: { num: [5, 6, 7], originUrl: this.$route.fullPath } //1是第1张图片
        });
      }
    },
    async nextPage() {
      this.curPageNum++;
      if (this.curPageNum > Math.ceil(this.comNum / 10)) {
        return this.$toast("没有更多数据了");
      }
      let postData = {
        provinces: this.selectedProvince,
        industryid: this.industry.id,
        business: this.business,
        page: this.curPageNum
      };

      let { data } = await this.getData(postData);

      this.resultList = data.contacts;
    },
    selectProvince(area) {
      this.selectedCity = "全选";
      this.selectedProvince = area.join();
      this.isShowProvinceList = false;

      let selsectProvince = this.cityList.filter(item => {
        return area.includes(item.province);
      });
      let tem = [];
      selsectProvince.forEach(item => {
        item.info.forEach(c => {
          let obj = {};
          obj["id"] = c.city_id;
          obj["label"] = c.city;

          tem.push(obj);
        });
      });

      this.cities = tem;
      let obj = { id: "", label: "全选" };
      this.cities.unshift(obj);
    },

    //选择行业  -- 此方法只能单选，现在要多选
    // selectIndustry() {
    //   this.$weui.picker(this.industryList, {
    //     className: "custom-classname",
    //     container: "body",
    //     defaultValue: [3],
    //     onChange: function(result) {
    //       console.log(result);
    //     },
    //     onConfirm: result => {
    //       this.industry.name = result[0].label;
    //       this.industry.id = result[0].id;
    //     },
    //     id: "singleLinePicker"
    //   });
    // },

    showSlectCity() {
      this.isShowSelectCity = !this.isShowSelectCity;
      this.isShowProvinceList = false;
      this.isShowIndustryList = false;
    },
    showSelectList() {
      this.isShowProvinceList = !this.isShowProvinceList;
      this.isShowSelectCity = false;
      this.isShowIndustryList = false;
    },
    showIndustryList() {
      this.isShowIndustryList = !this.isShowIndustryList;
      this.isShowSelectCity = false;
      this.isShowProvinceList = false;
    },

    async search() {
      let postData = {
        provinces:
          this.selectedProvince === "全国" ? "" : this.selectedProvince,
        industryid: this.selsectIndustryID,
        business: this.business,
        page: 1,
        city: this.selectedCity === "全选" ? "" : this.selectedCity,
        company_name: this.inputCompanyName
      };

      console.log(`====0===`, postData);

      if (!postData.company_name  &&  !postData.business) {
        return this.$toast("主营产品/服务 或 企业名称关键词 必须输入一个。");
      }
   

      let { data } = await this.getData(postData);
      console.log(`===1====`, data);

      if (data.message) {
        return this.$toast(data.message);
      }

      this.resultList = data.contacts;
      this.comNum = data.all_count;
      this.comPhoneNum = data.count;
    },

    async getIndustry() {
      try {
        let { data } = await this.$axios.get(this.$api.selectIndustry);
        this.industryList = data.map((ele, index) => {
          return {
            label: ele.industry,
            value: index,
            id: ele.id
          };
        });
        let tem = { id: "", label: "全行业" };
        this.industryList.unshift(tem);
      } catch (err) {
        this.$toast(err);
      }
    },
    async getAllCity() {
      let cityList = await this.$ajax.getAllCity();
      this.cityList = cityList;

      let arr = [];
      cityList.forEach(item => {
        let obj = {};
        obj["id"] = item.province_id;
        obj["label"] = item.province;
        arr.push(obj);
      });

      this.province = arr;
      this.province.unshift({ id: "", label: "全国" });
    },

    async getData(data) {
      let res = await this.$axios.get(`${this.$api.accSearch}`, {
        params: data
      });

      return res;
    },
    onFocus() {
      this.inputPlaceholder = "";
    },
    onBlur() {
      this.inputPlaceholder = "请输入主营产品/业务";
    }
  }
};
</script>

<style lang="stylus" scoped>
#msg-search
  display flex
  flex-direction column
  width 100%
  height 100%
  font-size 0.29rem
  background-color transparent
  // #select_style
  //   color black
  //   position absolute
  //   z-index 2
  $bgColor = #333333
  .mask
    position absolute 
    top 0
    left 0
    right 0
    bottom 0
    z-index 99
    background-color rgba(0,0,0,0.4)
  #main-body
    width 100%
    background-color #333333
    color white
    text-align center
    box-sizing border-box
    border-radius 6px
    .select-list
      padding 0 0.2rem
      .item
        position relative
        border-bottom 1px solid #545454
        display flex
        justify-content space-between
        span  
          height 1.1rem
          line-height 1.1rem
        .ri
          color #f3f3f3
          width 60%
          text-align right
          overflow hidden
          white-space nowrap 
          text-overflow ellipsis 
          &::after
            content ''
            display inline-block
            width 0.14rem
            height 0.24rem
            background url(../../assets/icon/arrow@16_29.png) center no-repeat
            background-size contain
            margin-left 0.3rem
        .item-input
          border 0
          background-color transparent
          position relative
          color #f3f3f3
          width 60%
          font-size 0.32rem
          &::placeholder
            text-align right
            font-size 0.28rem
      // .select-list
      //   width 98%      
    .area-box
      position absolute 
      width 100%
      left 0
      z-index 100
      display inline-block
      transition 0.2s
      transform-origin 0 0
      transform scaleY(0)
    .searchBtn
      position relative
      width 90%
      padding 0.24rem 0.6rem
      border-radius 0.4rem
      background-color #09a3a3
      font-size 0.32rem
      color #fff
      margin-top 0.3rem
      margin-bottom 0.3rem
  .result-list
    flex 1
    width 100%
    // height auto
    overflow-y scroll
    margin-top 0.15rem
    background $bgColor
    padding 0 0.2rem 0.2rem
    box-sizing border-box

    .total
      display flex
      justify-content space-between
      align-items flex-end
      font-size 0.28rem
      box-sizing border-box
      .num
        font-weight 600
        font-size 0.32rem
        color white

      .next-page
        color   #09a2a3
        color 0.28rem
    .list
      color silver
      display flex
      flex-direction column
      margin-top 0.2rem
      box-sizing border-box
      .item
        display flex
        .text
          display flex
          .item-name 
            max-width 60vw
            overflow hidden
            white-space nowrap 
            text-overflow ellipsis 
          .item-phone
            max-width 30vw
  .btn
    // position absolute 
    width 100%
    bottom 0
    display flex
    justify-content space-around
    align-items center
    // border-top 1px solid #ccc
    height 0.8rem
    border-radius 0 0 6px 6px

    font-size 0.3rem
    background #666666

    .btn-text, .btn-video
      width 100%
      height 100%
      line-height 0.8rem
      text-align center
      // border-radius 0.6rem
      color #fff
    .btn-text
      background-color transparent
      &::before
        vertical-align middle
        display inline-block
        width 0.32rem
        height 0.32rem
        margin-right 0.1rem
        margin-bottom 0.05rem
        content ''
        background url(../../assets/icon/chuke/msg.png) center no-repeat
        background-size contain
        
    .btn-video
      background-color #ffa200
      &::before
        vertical-align middle
        display inline-block
        width 0.32rem
        height 0.32rem
        margin-right 0.1rem
        margin-bottom 0.05rem
        content ''
        background url(../../assets/icon/chuke/vedio.png) center no-repeat
        background-size contain






</style>


