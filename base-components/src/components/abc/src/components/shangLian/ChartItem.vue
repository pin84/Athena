      ...
      handler: function(newCId) {
        if (newCId != "") {
          ...
          function loginFresh() {
            getCheckdimension()
              .then(checkState => {
                console.log(checkState, "checkState checkState ------");
                ...
                if (rows != null) {
                  ...
                  if (checkStateRes.IsResult) {
                    ...
                    Object.keys(listinfo).forEach((keyItem, index) => {
                      ...
                      // console.log(title,titleid,state)
                      ...
                      if (
                        ...
                        // console.log('test test ',index%iconList.length,iconItem)
                      }
                    ...
                    // console.clear()
                    // console.log(saveInfoList,'saveInfoList saveInfoList');

                    vm.$set(vm, "saveInfoList", saveInfoList);
                  }

                  // console.log(transCheckState,'transCheckState'); #$mount
                  ...
                  GetFirstSave()
                    .then(res => {
                      ...
                      // console.log('run!!! AGAIN' ,res, res.IsResult, res.Result)
                      if (IsResult && Result === null) {
                        // console.log('run!!! success' ,res, res.IsResult, res.Result)
                        vm.newUser = false;
  ...
  methods: {
    ...
    getMainFnBtn(width, height, id) {
      ...
      function getM(p2, $data, that) {
        ...
        console.log(titleCon, "titleCon titleCon titleCon");
        ...
        .then(
          vm.$axios.spread((up, down, visit) => {
            ...
            if (down.data.IsResult) {
              ...
              // vm.streamData.downData.data = downRes.rows.Down.length > 3 ? downRes.rows.Down.slice(0, 4) : downRes.rows.Down;
    ...
    getEnterpriseList(id) {
      ...
      //初始化随机获取企业
      // 假如已存在企业信息ID
      // console.log(id,'ALLgetEnterpriseListALLgetEnterpriseListALLgetEnterpriseListALLgetEnterpriseList')
      if (id) {
    ...
    checkDimensionInfo(id) {
      ...
      vm.mainFnBtnList.forEach((btnItem, i) => {
        /* !废弃
                for (var j in obj) {
                    // console.log('检查企业维度的值,改变自画像的显示值 j ',j,v.en,v.en == j)
                    if (btnItem.en == j) {
                        btnItem.isCheck = obj[j];
                    }
                };
                */
        //    console.log(btnItem,'btnItem------btnItem')
        //    console.dir(btnItem,'btnItem--------btnItem dir ')
        // 应该是该维度按钮是否被选择吧?
        if (btnItem.isCheck) {
          for (var k = 0; k <= 2; k++) {
            var o = {};
