...
export default {
  ...
  data() {
    return {
      ...
      tdata: [], //从后台取回来的下链数据，暂时存在这个字段
      tCompany: {}, //从后台取回来的公司的数据，暂时存在这个字段
      industryid: "" //行业的id,传给一个页面用
    };
  },
  ...
  beforeCreate() {
    ...
    let isMock = true;
    this.$axios.get(`http://192.168.31.226:8000/index/info/老王`).then(res => {
      this.tCompany = res;
    ...
    //下链数据;
    this.$axios.get("http://192.168.31.226:8000/index/industry").then(res => {
      this.tdata = res;
    });
  },
  beforeMount() {
  ...
  mounted() {
    ...
    //公司的基本信息
    // this.$axios.get("http://192.168.31.226:8000/index/industry?enterpriseid=0be46c589062385fa8094eb57dd0afa1&industryid=4").then(res => {
    //   this.tdata = res;
    // });
  },
  ...
  methods: {
    ...
    makeEnterpriseList(id) {
      ...
      // 访问数 推荐数 公司名接口
      vm.streamData.title = this.tCompany.data.enterprise;
      vm.streamData.visit = this.tCompany.data.count
        ? this.tCompany.data.count
        : 0;
      vm.streamData.rec = this.tCompany.data.number
        ? this.tCompany.data.number
        : 0;
      //以下两个字段是为了传给下一个页面。而取的数据
      vm.cID = this.tCompany.data.enterpriseid;
      vm.industryid = this.tCompany.data.industryid;
          ...
          vm.$axios.spread((up, down, visit) => {
            ...
            if (up.data.IsResult) {
              ...
              upRes.rows.Up.length > showNum
                ? (showRes = upRes.rows.Up.slice(0, 4))
