<template>
  ...
  <!-- <div class="lianQuan"> -->
    ...
    >
      ...
      <symbol id="svg-coor-icon"  class="icon" viewBox="0 0 1024 1024" width="200" height="200">
        <path d="M514.327273 26.065455c-214.109091 0-387.258182 172.218182-387.258182 385.396364 0 97.745455 38.167273 188.974545 98.676364 256.930909l251.345455 316.509091c19.549091 25.134545 52.130909 25.134545 71.68 0l250.414545-316.509091c63.301818-68.887273 102.4-156.392727 102.4-256.930909C901.585455 199.214545 728.436364 26.065455 514.327273 26.065455L514.327273 26.065455z" p-id="2102" fill="#d81e06"></path>
      </symbol>

      <symbol id="svg-hot-icon" class="icon" viewBox="0 0 1296 1024" width="200" height="200">
        <path d="M698.406 182.739c24.135 51.718 27.583 137.916-20.688 179.29C598.417 58.615 401.886 0 401.886 0c24.136 155.156-82.75 324.103-186.186 451.675-3.448-62.062-6.896-103.437-41.375-165.5-6.896 113.782-93.094 203.427-117.229 317.208C26.065 758.538 81.231 868.87 291.553 989.548c-0.417-0.889-0.821-1.764-1.239-2.653C340.201 1010.37 399.597 1024 463.342 1024c99.909 0 189.096-33.455 247.778-85.874 403.149-322.595 52.688-709.608-12.714-755.387zM393.24 741.797h-38.385v-81.281h-76.77v81.281H239.7V544.742h38.385v79.503h76.77v-79.503h38.385v197.055z m182.092-25.253c-16.324 19.125-37.712 28.688-64.15 28.688-25.819 0-46.802-9.267-62.924-27.799-16.135-18.532-24.19-42.385-24.19-71.557 0-30.87 8.243-55.988 24.715-75.37s38.25-29.064 65.308-29.064c25.752 0 46.466 9.374 62.17 28.108 15.704 18.748 23.556 42.83 23.556 72.258 0 30.708-8.161 55.611-24.485 74.736zM744.292 579.1H692.87v162.698h-38.506V579.1h-51.3v-34.344h141.215V579.1z" fill="#D81E06" p-id="5566"></path><path d="M512.933 577.82c-15.044 0-26.896 6.101-35.53 18.317-8.646 12.216-12.97 28.028-12.97 47.449 0 19.165 4.23 34.843 12.674 47.005 8.445 12.175 20.041 18.25 34.775 18.25 15.045 0 26.749-5.833 35.126-17.496 8.377-11.664 12.566-27.328 12.566-47.005 0-20.526-4.068-36.742-12.216-48.661-8.135-11.893-19.623-17.86-34.425-17.86z" fill="#D81E06" p-id="5567"></path>
      </symbol>
    </svg>
    ...
    <div v-show="pieMapCutBtn" @click="cutPieMap" class="iconfont shwotype-btn" >
      <span class="icon">{{isShowMapPlus?'&#xe609;':'&#xe618;'}}</span> {{isShowMapPlus?'饼图占比':'地理分布'}}
    </div>

    <div v-show="pieMapCutBtn" class="click_tips">
      <div class="click_img_content">
        <img class="click_img" src="@/assets/icon/toClick.png">
      </div>
      点击{{isShowMapPlus?'数字':'饼图'}}进入
    </div>
    <div v-show="pieMapCutBtn && showingPieDataType == 'special'" class="special-tips">
      <div v-for="(spText,index) in specialTips" :key="index" class="dialog-box">
        <div :class="{'hid_text':spText.popFlexState}" v-html="spText.tipsText"></div>
        <div @click="spText.popFlexState = !spText.popFlexState" :class="{'turn':spText.popFlexState}" class="iconfont content_flex">&#xe602;</div>
      </div>
      <!-- <div class="dialog-box">{{'test'}}</div> -->
    </div>
    <!-- <div v-show="isShowMapPlus && pieMapCutBtn" class="unit" >单位：万</div> -->
...
let vm = null,
  d3 = null;
// let provinceAllNameList = ["上海市", "云南省", "内蒙古自治区", "北京市", "吉林省", "四川省", "天津市", "宁夏回族自治区", "安徽省", "山东省", "山西省", "广东省", "广西壮族自治区", "新疆维吾尔自治区", "江苏省", "江西省", "河北省", "河南省", "浙江省", "海南省", "湖北省", "湖南省", "甘肃省", "福建省", "西藏自治区", "贵州省", "辽宁省", "重庆市", "陕西省", "青海省", "黑龙江省"]
...
// chinaGEOJSON.features.forEach(ele=>{
//   let index = provinceAllNameList.findIndex(elep=>{
//     elep.slice(0,2) == ele.properties.name.slice(0,2)?true:false
//     // 
//     return elep.slice(0,2) == ele.properties.name.slice(0,2)?true:false
//   })
  
//   let allName = provinceAllNameList[index]
//   ele.properties.allName = allName
// })
// console.log(JSON.stringify(chinaGEOJSON))
// 
export default {
  name: "NewChart",
  components: {
  ...
  data() {
    return {
      ...
      showingPieDataType:null, //正在显示的饼图的类型 up / down / special
      ...
      specialTips:[
        {
          tipsText:`欢迎来到上下链法律服务专场。我们在全国各地挑选各项专项服务突出的律师所认证到平台来，服务上下链八千多万的中小企业，解决他们在日常法律顾问、债权债务、知识产权、融资股权等各种经营上的实际问题。`,
          popFlexState:false,
        }
        ,
        {
          tipsText:`上下链提供线上咨询、企业与律所对接合作关系，提供了企业的法律服务专场。<span class="key">认证律师所九大特权。(点击下图)</span>`,
          popFlexState:false,
        }
  ...
  async mounted(){
    ...
    let {downData, upData, specialData,} = await this.getNeedInfo();
    
     // 弹出分享时出现的公司弹窗
    let companyInfo = null;
    companyInfo = await this.popHrefBaseInfo();
    
    let willUseCompanyInfo = companyInfo ? companyInfo:this.$store.state.company.indexInfo;
    this.companyInfo = willUseCompanyInfo;
    this.createInit(downData,upData,specialData);
  ...
  computed:{
    ...
    mapPlusCanMove(){
      ...
      if( this.isShowMapPlus && this.isPlus ){
        return true
      }else{
        return false
      }
  ...
  watch:{
    // 为防止在隐藏阶段仍移动地图而设置条件 非显示阶段移除事件
    mapPlusCanMove(newVal){
      if(newVal){
        this.mapPlusZoom()
      }else{
        this.mapPlusZoomClear()
      }
    }
  ...
  methods: {
    ...
    cutPieMap(){
      ...
      this.isShowMapPlus = !this.isShowMapPlus;
      const nodePlus = d3.select('#node-plus-group')
      const mapGPlus = d3.select('#mapGPlus')
      // const coorIcon = d3.select('#coor-icon')

      if(this.isShowMapPlus){
        nodePlus.style('display','none');
        mapGPlus.style('display','block');
        // coorIcon.style('display','block');
        // this.mapPlusZoom();
      }else{
        nodePlus.style('display','block')
        mapGPlus.style('display','none');
        // coorIcon.style('display','none');
        // this.mapPlusZoomClear();
      }
    ...
    createMap(mapId='mapG',isShow=true,hasProvinceName=true){
    
      const baseLine = this.svgWidth > this.svgHeight ? this.svgHeight : this.svgWidth
      const projection = d3.geoMercator()
                          .scale(baseLine*0.9) //投影比例因子 按比例放大投影
                          .center([105, 16]) //中国地图经纬度中心点
                          .translate([this.svgWidth / 2, this.svgHeight *0.98]);
                              //  .translate([0, 0]);
      ...
      const mapBlock = mapBlockG.append('path')
                                .attr('d', path)
                                .attr('fill', '#e5a21f')
                                .attr('stroke', '#946000')
                                .attr('stroke-width', 1)

      const mapText = mapG.append('g')
    ...
    createMapPlus(){
      const { mapG, mapBlockG,mapBlock, mapText,projection } = this.createMap('mapGPlus',false);
      
      const coorWidth = 0.6;
      const coorHeight = 0.20;
      const provinceFS = 0.16;
      
      const coorPointG = mapG.append('g')
                            .attr('class','coor-group')
      /*
      const coorItemG = coorPointG.selectAll('text')
                                  .data(chinaGEOJSON.features)
                                  .enter()
                                  .append('g')
                                  .attr('class','coor-item')
                                  .style('transform',(d)=>{
                                    let [ log, lat ] = d.properties.cp;
                                    let coor = projection([ log, lat ])
                                    // coor[1] = coor[1];
                                    ...
                                    // let provinceName = vm.provinceTrans(vm.companyInfo.province);
                                    // d.properties.name === provinceName ? this.targetProvince = d.properties.coor : null;
                                    return `translate(${coor[0]}px,${coor[1]}px)`;
                                  })
                                  .style('display',(d)=>{
                                    let ignoreProvince = ['香港','澳门','台湾'];
                                    if(ignoreProvince.includes(d.properties.name)){ return 'none'}
                                  })
      coorItemG.append('use')
               .attr('href','#svg-coor-icon')
               .attr('width',`${coorWidth}rem`)
               .attr('height',`${coorHeight}rem`)
      coorItemG.append('text')
               .attr('class','map-province-val')
               .style('fill','white')
               .style('transform',(d)=>{
                 return `translate( 0.24rem , 0.24rem )`
               })
               .attr('text-anchor','middle') //水平居中
              //  .attr('dominant-baseline','middle') //垂直居中

      mapG.select('.coor-group')
          .style('transform','translate(-0.24rem,-0.6rem)')
      */
     ...
     const coorItemG = coorPointG.selectAll('text')
      ...
      const rightPosition = ['天津','福建','浙江','山东','河北','上海','江苏','江西'];
      const leftPosition = ['云南','四川','陕西'];

      coorItemG.append('rect')
               .attr('width',`${coorWidth}rem`)
               .attr('height',`${coorHeight}rem`)
               .attr('rx','2px')
               .attr('ry','2px')
               .style('transform',(d)=>{
                 if(rightPosition.includes(d.properties.name)){
                   return `translate( ${ coorWidth * 0.33 }rem , ${ -coorHeight * 0.8}rem )`
                 }
                 if(leftPosition.includes(d.properties.name)){
                   return `translate( ${ -coorWidth -coorWidth * 0.33 }rem , ${ -coorHeight * 0.8}rem )`
                 }
                //  
                 return `translate( ${-coorWidth / 2 }rem , ${-provinceFS - coorHeight}rem )`
               })
               .style('fill','red')
      ...
      coorItemG.append('text')
               ...
               .style('transform',(d)=>{
                 if(rightPosition.includes(d.properties.name)){
                   return `translate( ${ coorWidth * 0.8 }rem , ${ -coorHeight * 0.25 * 0}rem )`
                 }
                 if(leftPosition.includes(d.properties.name)){
                   return `translate( ${ -coorWidth * 0.8 }rem , ${ -coorHeight * 0.25 * 0}rem )`
                 }
                
                //  -provinceFS -provinceFS * 0.5
                 return `translate( ${0}rem , ${ -coorHeight }rem )`
               })
    ...
    createInit(downData,upData,specialData){
      ...
      this.$nextTick()
          .then(()=>{
            ...
            let {
            getNodes,simulation,graphData,link,
            industryNodeG,mainNodeG,svgWidth,svgHeight
            } = this.init(newJson)  //初始化 主要是初始化图谱
            ...
            this.createMapPlus();
            this.eventManage(getNodes,simulation,graphData,link,industryNodeG,mainNodeG,svgWidth,svgHeight)
      ...
      if( logoType == 'special'){
        ...
        this.$router.push({
          ...
          query:{
            ...
            specialType:industryName,
          }
        })
        // this.$toast('专场即将开放，敬请期待')
        return
      }
    ...
    async choosePie(dataType,btnItem){
      ...
      if(vm.showingPlusPie[0] == showingPlusPie){
        return;
      }
      vm.showingPlusPie[0].style('display','none');
      vm.showingPlusPie.length = 0;
      vm.showingPlusPie.unshift(showingPlusPie);
      vm.showingPieInfo = {
    ...
    changeMapValue(){
      const coorWidth = 0.6;
      const coorHeight = 0.2;

      let provinceAllData = null;
      vm.showingPlusPie[0].datum(d=>{
        provinceAllData = d.provinceAll;
        return d
      })
      d3.selectAll('.map-province-val')
        .html(d=>{
          let findIndex = provinceAllData.findIndex(ele=>{
            ele.Name.slice(0,2) === d.properties.name.slice(0,2)
            
            return ele.Name.slice(0,2) === d.properties.name.slice(0,2)
          })
          // 
          if(findIndex !== -1){
            // 
            let count = provinceAllData[findIndex].Count
            count = vm.$commonFn.floatObj.divide(count,10000).toFixed(2)+'万'
            d.textLen = count.length
            return count
          }
          
        })
        .style('font-size',d=>{
          // return ( coorWidth / d.textLen ) + 'rem'
          return coorHeight * 0.8 + 'rem'
        })
    ...
    async getNeedInfo(){
      // 请求分别获取上下链数据
      let downDataReq = []
      downDataReq = this.$axios.get(this.$api.downData,{
        params:{
          num:12,
        }
      }).then(res=>{
      ...
      let upDataReq = [];
      /*
      upDataReq = this.$axios.get(this.$api.upData).then(res=>{
        if(res.data.message){
          this.$toast({
      ...
      */
      ...
      // if(this.specialModel){
      //   // 专场接口
        
      //   // debugger
      //   for(let specialKey in this.specialDict){
      //     this.specialDict[specialKey]
      //   }
        
      //   specialReq = this.$axios.get(this.$api.speciaIndustry,{
      //     params:{
      //       kind:'律师事务所',
      //     }
      //   }).then(res=>res.data)
      //   specialData = await specialReq;
      //   let findItem = specialData.findIndex(ele=>{
      //     return ele.ClassName.Name === "律师事务所"
      //   })
      //   specialData[findItem].ClassName.Name = '法律服务';
    ...
    zoomRestore(simulation=this.simulation){
      ...
      this.pieMapCutBtn = false;
      try{
        this.showingPlusPie.forEach(ele=>{
          ele.style('display','none')
        })
        this.showingPlusPie.length = 0;
      }catch(e){
      }
      d3.select('#mapGPlus').style('display','none')
      // d3.select('#coor-icon').style('display','none')
    ...
    // 上下链数据转化为对应力导图所需数据  
    json2forceData(downJson,upJson,specialData=[]){
      // dataType 有两种
      //  down up 
      let forceData={};
      forceData.links = [];
      ...
      // 将以下数据深克隆 避免影响原始数据
      let showUp = JSON.parse(JSON.stringify(upJson)) //用于展示的上链数据
      let showDown = JSON.parse(JSON.stringify(downJson)) //用于展示的下链数据
      let showSpecial = JSON.parse(JSON.stringify(specialData)) //用于展示专场数据

      // let showSpecial;
      // let specialForceData = {
      //   links:[],
      //   nodes:[],
      // };
      // if(specialData !== null){
      ...
      let specialForceData = this.getPieNodeData(showSpecial,'special') //获得转换格式后的专场数据
      
      forceData.links = [...upForceData.links,...downForceData.links,...specialForceData.links]
      forceData.nodes = [...upForceData.nodes,...downForceData.nodes,...specialForceData.nodes]
    ...
    async getIndustryInfo(dataType,industryName,industryID){
      ...
      let getRes 
      if(['up','down'].includes(dataType)){
        getRes = await this.$axios.get(this.$api.singleData,{
          params:{
            ...sendParams,
          },
        }).then(res=>res.data)
      ...
      if(dataType == 'special'){
        let resItem;
        resItem = this.specialData.findIndex(ele=>{
          return ele.ClassName.Name === industryName
        })
        let specialInfo;
        if(resItem == -1){
          specialInfo = await this.$ajax.getSpecialIndustry(industryName);
          this.specialData = specialInfo
          resItem = 0;
        }

        getRes = JSON.parse(JSON.stringify(this.specialData[resItem]));
       
        let newJson = this.getPieNodeData([getRes],dataType)
    ...
    init(graphData) {
      ...
      simulation.nodes(graphData.nodes).on("tick", ()=>vm.ticked(link,industryNodeG,mainNodeG,svgWidth,svgHeight));
      ...
      return {getNodes,simulation,graphData,link,industryNodeG,mainNodeG,svgWidth,svgHeight};
    },
    ...
    initNodes(graphData,simulation) {
      ...
      .attr('transform',function(d){
        ...
        return `translate(${ -((d.industryName+'专场').length * textFontSize + textFontSize ) / 2 },${d.radius + 5 })`
        
      })
      .style('fill','rgba(206, 8, 8, 0.8)')
      .attr('width',(d)=>{
        return (d.industryName+'专场').length * textFontSize + textFontSize;
      })
      .attr('height',(d)=>{
      ...
      nodeText.style('font-size',textFontSize)
              ...
              .attr('transform',function(d){
                d.industryFontSize = textFontSize;
                if(d.dataType == 'special'){
                  return `translate(0,${d.radius + textFontSize * 1.4 + 5})`
                }
                return `translate(0,${d.radius + textFontSize})`
              })
              .html(function(d){
                if(d.dataType == 'special'){
                  return d.industryName+'专场'
                }
    ...
    initPieNode(industryPie){
      ...
      let centerCircle  = centerCircleG.append('circle')
                                    ...
                                    .style('fill',function(d){
                                      ...
                                      else if(d.dataType == 'up' || d.dataType == 'special'){
                                        return 'white'
                                      }
        ...
        */
        ...
        // svg-hot-icon
        /*
        centerCircleG.append((d)=>{
                        if(d.dataType == 'down' || d.dataType == 'up'){
                     ...
                     .style('font-size',innerWidth/3.4)
                     .style('font-family','TRENDS')
                     .attr('text-anchor','middle') //水平居中
                     .attr('dominant-baseline','middle') //垂直居中
                     .attr('transform',(d)=>{
                       if(d.dataType == 'down'){
                         return `translate(0,${-innerWidth/3.6})`
                       }
                       if(d.dataType == 'special'){
                         return `translate(${ ( -innerWidth / 2.4 ) / 2 },${ (-innerWidth/2) })`
                       }
                     })
        ...
        centerCircleG.append('text')
                     ...
                     .style('font-size',(d)=>{
                       if(d.dataType == 'down'){
                        ...
                        return innerWidth / (d.industryName.length)
                       }
                     })
                     .style('font-family','TRENDS')
                     .attr('text-anchor','middle') //水平居中
        ...
        centerCircleG.append((d)=>{
                    ...
                    .attr('transform',(d)=>{
                        if(vm.specialModel){
                          ...
                          return `translate(0,${-innerWidth / 6})`
                        }
                        if(d.dataType == 'down' || d.dataType == 'up'){
                          return `translate(0,${-innerWidth/3.6})`
                        }
        ...
        centerCircleG.append('text')
                    ...
                    .style('font-size',(d)=>{
                      if(vm.specialModel){
                        ...
                        if(d.dataType == 'down'){
                          return innerWidth/((`"下"为企业客户资源`).length-2);
                        }
                        if(d.dataType == 'up'){
                          return innerWidth/((`"上"为企业供应资源`).length-2);
                        }
                      }
                    })
                    .style('font-family','TRENDS')
      ...
      svg.on('click',function(){
        ...
        switch(Target){
          ...
          case vm.bgShade.node(): // 背景遮罩
            ...
            vm.zoomRestore(simulation); // 恢复回初始状态
        ...
        if(Target.__data__.type === "Feature"){
          ...
          vm.provincePop(Target.__data__.properties.allName)
    ...
    mapPlusZoom(){
      ...
      let zoom = d3.zoom()
                ...
                .scaleExtent([1, 8]) //指定伸缩区间
                .on("zoom",zoomed)

      let mapGplus = d3.select('#mapGPlus')
      // let coorIcon = d3.select('#coor-icon')
      let coorItem = d3.selectAll('.coor-item')
      function zoomed() {
        if(vm.isShowMapPlus && vm.isPlus){
          let transform = d3.zoomTransform(this);
          mapGplus.attr("transform", d3.event.transform);
        }
      }
    ...
    mapPlusZoomClear(){
      const svg = d3.select('svg')
      const STOPEVENTNAME = ['mousedown','mousemove','dblclick','touchstart','wheel','mousewheel','MozMousePixelScroll',]
      STOPEVENTNAME.forEach(ele=>{
        svg.on(ele+'.zoom',null)
      })
    },
    ...
    async nodePlus(plusItem,simulation,hrefPieInfo=null){
      ...
      ;
      if( this.isShowMapPlus ){
        d3.select('#mapGPlus').style('display','block');
      }
      ...
      if(pieType == 'main'){
        ...
        // 上下链节点放大
        this.pieMapCutBtn = true;
        if(this.isShowMapPlus){
          // d3.select('#coor-icon')
          //   .style('display','block');
        }
        // 选择行业饼图
        this.pieShareInfoSet(dataType,industryName,industryID) //微信分享
        // 是否有对对应饼图做缓存处理
        ...
        // 饼图选择按钮组
        if(dataType=='up'){
          vm.industryList = upIndustry ;

        }
        if(dataType=='down'){
          vm.industryList = downIndustry;
  ...
  activated(){
    let searchCompany = this.$store.state.company.searchCompany;
    
    if(searchCompany !== false){
      
      this.$nextTick(()=>{
        try{
            this.zoomRestore();
            this.changeCompany(...searchCompany.params)
            this.$store.commit('searchCompany',false);
        }catch(e){
          console.log(e);
        }
    ...
    if(this.pieIsReady){
      this.createHrefPie(this.simulation);
    }
  },
  deactivated(){
...
<style lang="scss" scoped >
  ...
  .dialog-box{
    ...
    &::before{
      ...
      border:{
        left:$triangleSide solid $bgColor;
        bottom:$triangleHeight /2  solid $bgColor;
        top:$triangleHeight /2 solid transparent;
        right:$triangleSide solid transparent;
  ...
  .shwotype-btn{
    ...
    bottom: 2%;
    height: $height;
    padding:$topBtmHeight 0.14rem;
    border-radius: ( $height + $topBtmHeight * 2 ) / 2 ;
    color:white;
    font-size: 0.36rem;
    background: #09a2a3;
    line-height: $height ;
