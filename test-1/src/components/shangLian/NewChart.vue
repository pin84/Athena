...
export default {
  ...
  beforeCreate() {
    ...
    this.$axios.get(this.$api.tokenTest,{
      params:{
  ...
  async created() {
    ...
    // 请求分别获取上下链数据
    let downDataReq = this.$axios.get(this.$api.downData)
    let downData = await downDataReq.then(res=>{
      if(res.data.message){
        this.$toast({
    ...
    let upDataReq = this.$axios.get(this.$api.upData)
    let upData = await upDataReq.then(res=>{
      if(res.data.message){
        this.$toast({
  ...
  mounted(){
    ...
    //   this.activatedFunc = {};
    //   console.log('changeCompany first listen')
