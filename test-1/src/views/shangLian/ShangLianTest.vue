    ...
    >
      ...
      <div class="index-serNote-con fr">
        ...
        <p class="index-serNote-con-word main_color_ffffff">
          1、认证您的企业，可面向几
          千万企业资源匹配推荐。图谱触控式可视化，一眼揽括企业上下游企业商机资源，即见即可得，总揽数据全局。<br />
          2、上下链每天新发布几万条全网采购信息，免费订阅即可获得平台精准匹配推送到关注公众号的手机微信。<br />
          3、上下链提供AI系统辅助算法，可进行精准短信触客、分发触客。<br />
          4、企业可发布企业各种采购需求，精准推荐到供应商家。<br />
          5、提供图谱触控式CRM，建立客户漏斗模型，形成营销资源链圈。<br />
          6、邀请同事同步管理，企业AI营销渠道组织重构。<br />
          <!-- 7、邀请同事同步管理，企业AI营销重构。 -->
        </p>
...
import NewChart from "@/components/shangLian/NewChart";
...
export default {
  ...
  beforeCreate(){
    ...
    this.$axios
      .get(this.$api.isShowTips, {
        params: data
      })
      .then(res => {
        if (res.data === 0) {
          this.$router.push("tipssl");
        }
      })
      .catch(rej=>{
        this.$toast({
          message:'isShowTips错误'
        })
      })
      ;
    //第一次到访提示 e
  ...
  methods: {
    ...
    async posterActivity(){
      ...
      if(qs.indexOf(undefined)!==-1) {return}
      if(linkPop !== 'poster'){return}

      let userId = this.$store.state.loginInfo.userInfo.user_id;
      userId = +userId
      share_user = +share_user
      if(share_user == userId) { return }
      let posterReturn = await this.$axios.post(this.$api.posterActivity,{
